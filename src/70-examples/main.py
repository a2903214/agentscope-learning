...
async def run_stock_hot_topic_case(topic: str, horizon: str, risk: str) -> dict[str, Any]:
    ...
    def capture_tool_call(tool_call: ToolUseBlock, tool_resp: ToolResponse) -> ToolResponse:
        ...
        log.info(
            "[tool_call] name=%s input=%s result_snippet=%s",
            tool_call["name"],
            inp,
            snippet,
        )
    ...
    log.info("agent call start topic=%s horizon=%s risk=%s", topic, horizon, risk)
    response = await agent(Msg(name="user", role="user", content=user_prompt))
    log.info("agent call done, parsing reply and report")
...
async def main() -> None:
    ...
    print(f"top_pick  : {top_pick.get('symbol', 'N/A')}")
